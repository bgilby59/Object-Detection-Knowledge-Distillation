...
vgg_, ssd_, head_ = multibox(vgg16(vgg, 3),
                                     add_extras(ssd, 1024),
                                     mbox, num_classes=201) # CHANGES NUM CLASSES FOR COCO
...
class vgg_ssd(nn.Module):
    def __init__(self, backbone, ssd, head, mode, num_classes=21):
        ...
        self.mode = mode
        self.priorbox = PriorBox(coco_conf)
        with torch.no_grad():
...
def create_vgg(mode, num_classes=21):
    return vgg_ssd(vgg_, ssd_, head_, mode, num_classes=num_classes)